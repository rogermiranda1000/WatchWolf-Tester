...
public class StrippedJungleWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedJungleWood block doesn't allow the direction " + d.name());
		StrippedJungleWood current = new StrippedJungleWood(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedJungleWood(short id) {
		super(id, "STRIPPED_JUNGLE_WOOD", (ins, f) -> {
