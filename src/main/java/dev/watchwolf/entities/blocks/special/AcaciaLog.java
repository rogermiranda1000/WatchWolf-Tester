...
public class AcaciaLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("AcaciaLog block doesn't allow the direction " + d.name());
		AcaciaLog current = new AcaciaLog(this);
		current.direction = d;
		return current;
	}
	@Override
