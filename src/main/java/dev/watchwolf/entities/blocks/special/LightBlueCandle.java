...
public class LightBlueCandle extends Block implements Groupable, Ignitable {
	/*   --- GROUPABLE INTERFACE ---   */
	...
	public Groupable setGroupAmount(int amount) throws IllegalArgumentException {
		if (amount < 1 || amount > this.getMaxGroupAmount()) throw new IllegalArgumentException("LightBlueCandle block only allows grouping from 1 to " + this.getMaxGroupAmount());
		LightBlueCandle current = new LightBlueCandle(this);
	...
	public LightBlueCandle(short id) {
		super(id, "LIGHT_BLUE_CANDLE", (ins, f) -> {
