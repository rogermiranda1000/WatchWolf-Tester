...
public class PolishedDioriteStairs extends Block implements Orientable, Sectionable {
	...
	public Sectionable setSection(Sectionable.Section section) throws IllegalArgumentException {
		if (!this.allowedSections.contains(section)) throw new IllegalArgumentException("PolishedDioriteStairs block doesn't allow the section " + section.name());
		PolishedDioriteStairs current = new PolishedDioriteStairs(this);
	...
	/*   --- CONSTRUCTORS ---   */
	public PolishedDioriteStairs(short id) {
		super(id, "POLISHED_DIORITE_STAIRS", (ins, f) -> {
