...
public class JungleLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("JungleLog block doesn't allow the direction " + d.name());
		JungleLog current = new JungleLog(this);
		current.direction = d;
		return current;
	}
