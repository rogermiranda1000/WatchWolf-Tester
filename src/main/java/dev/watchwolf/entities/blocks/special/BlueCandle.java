...
public class BlueCandle extends Block implements Groupable, Ignitable {
	/*   --- GROUPABLE INTERFACE ---   */
	...
	public Groupable setGroupAmount(int amount) throws IllegalArgumentException {
		if (amount < 1 || amount > this.getMaxGroupAmount()) throw new IllegalArgumentException("BlueCandle block only allows grouping from 1 to " + this.getMaxGroupAmount());
		BlueCandle current = new BlueCandle(this);
