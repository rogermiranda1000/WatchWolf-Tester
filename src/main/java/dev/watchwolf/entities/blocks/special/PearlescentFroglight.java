...
public class PearlescentFroglight extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("PearlescentFroglight block doesn't allow the direction " + d.name());
		PearlescentFroglight current = new PearlescentFroglight(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public PearlescentFroglight(short id) {
		super(id, "PEARLESCENT_FROGLIGHT", (ins, f) -> {
