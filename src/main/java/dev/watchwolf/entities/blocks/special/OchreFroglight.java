...
public class OchreFroglight extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("OchreFroglight block doesn't allow the direction " + d.name());
		OchreFroglight current = new OchreFroglight(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public OchreFroglight(short id) {
		super(id, "OCHRE_FROGLIGHT", (ins, f) -> {
