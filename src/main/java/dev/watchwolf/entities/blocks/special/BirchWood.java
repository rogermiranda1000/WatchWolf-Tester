...
public class BirchWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("BirchWood block doesn't allow the direction " + d.name());
		BirchWood current = new BirchWood(this);
		current.direction = d;
		return current;
	}
	@Override
