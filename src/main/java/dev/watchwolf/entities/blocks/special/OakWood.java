...
public class OakWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("OakWood block doesn't allow the direction " + d.name());
		OakWood current = new OakWood(this);
		current.direction = d;
		return current;
	}
	@Override
