...
public class StrippedAcaciaLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedAcaciaLog block doesn't allow the direction " + d.name());
		StrippedAcaciaLog current = new StrippedAcaciaLog(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedAcaciaLog(short id) {
		super(id, "STRIPPED_ACACIA_LOG", (ins, f) -> {
