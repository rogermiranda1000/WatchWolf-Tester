...
public class SpruceWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("SpruceWood block doesn't allow the direction " + d.name());
		SpruceWood current = new SpruceWood(this);
		current.direction = d;
		return current;
	}
	@Override
