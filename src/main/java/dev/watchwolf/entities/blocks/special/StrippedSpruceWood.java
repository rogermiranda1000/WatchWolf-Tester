...
public class StrippedSpruceWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedSpruceWood block doesn't allow the direction " + d.name());
		StrippedSpruceWood current = new StrippedSpruceWood(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedSpruceWood(short id) {
		super(id, "STRIPPED_SPRUCE_WOOD", (ins, f) -> {
