...
public class VerdantFroglight extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("VerdantFroglight block doesn't allow the direction " + d.name());
		VerdantFroglight current = new VerdantFroglight(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public VerdantFroglight(short id) {
		super(id, "VERDANT_FROGLIGHT", (ins, f) -> {
