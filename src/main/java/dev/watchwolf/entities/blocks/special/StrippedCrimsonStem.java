...
public class StrippedCrimsonStem extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedCrimsonStem block doesn't allow the direction " + d.name());
		StrippedCrimsonStem current = new StrippedCrimsonStem(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedCrimsonStem(short id) {
		super(id, "STRIPPED_CRIMSON_STEM", (ins, f) -> {
