...
public class CrimsonHyphae extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("CrimsonHyphae block doesn't allow the direction " + d.name());
		CrimsonHyphae current = new CrimsonHyphae(this);
		current.direction = d;
		return current;
	}
	@Override
