...
public class Candle extends Block implements Groupable, Ignitable {
	/*   --- GROUPABLE INTERFACE ---   */
	...
	public Groupable setGroupAmount(int amount) throws IllegalArgumentException {
		if (amount < 1 || amount > this.getMaxGroupAmount()) throw new IllegalArgumentException("Candle block only allows grouping from 1 to " + this.getMaxGroupAmount());
		Candle current = new Candle(this);
