...
public class FrostedIce extends Block implements Ageable {
	/*   --- AGEABLE INTERFACE ---   */
	@RelevantBlockData
	private int age;
	...
	public Ageable setAge(int age) throws IllegalArgumentException {
		if (age > this.getMaxAge()) throw new IllegalArgumentException("FrostedIce block only allows age from 0 to " + this.getMaxAge());
		FrostedIce current = new FrostedIce(this);
		current.age = age;
		return current;
	}
	@Override
