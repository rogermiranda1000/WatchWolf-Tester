...
public class Beetroots extends Block implements Ageable {
	/*   --- AGEABLE INTERFACE ---   */
	@RelevantBlockData
	private int age;
	...
	public Ageable setAge(int age) throws IllegalArgumentException {
		if (age > this.getMaxAge()) throw new IllegalArgumentException("Beetroots block only allows age from 0 to " + this.getMaxAge());
		Beetroots current = new Beetroots(this);
		current.age = age;
		return current;
	}
	@Override
