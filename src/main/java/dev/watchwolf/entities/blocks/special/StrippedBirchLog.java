...
public class StrippedBirchLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedBirchLog block doesn't allow the direction " + d.name());
		StrippedBirchLog current = new StrippedBirchLog(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedBirchLog(short id) {
		super(id, "STRIPPED_BIRCH_LOG", (ins, f) -> {
