...
public class BirchLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("BirchLog block doesn't allow the direction " + d.name());
		BirchLog current = new BirchLog(this);
		current.direction = d;
		return current;
	}
	@Override
