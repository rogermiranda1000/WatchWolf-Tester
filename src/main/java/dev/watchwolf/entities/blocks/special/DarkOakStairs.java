...
public class DarkOakStairs extends Block implements Orientable, Sectionable {
	...
	public Sectionable setSection(Sectionable.Section section) throws IllegalArgumentException {
		if (!this.allowedSections.contains(section)) throw new IllegalArgumentException("DarkOakStairs block doesn't allow the section " + section.name());
		DarkOakStairs current = new DarkOakStairs(this);
	...
	public DarkOakStairs(short id) {
		super(id, "DARK_OAK_STAIRS", (ins, f) -> {
	...
	private DarkOakStairs(DarkOakStairs old) {
		this(old.id);
