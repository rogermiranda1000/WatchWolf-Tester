...
public class StrippedOakWood extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("StrippedOakWood block doesn't allow the direction " + d.name());
		StrippedOakWood current = new StrippedOakWood(this);
		current.direction = d;
		return current;
	}
	@Override
	...
	public StrippedOakWood(short id) {
		super(id, "STRIPPED_OAK_WOOD", (ins, f) -> {
