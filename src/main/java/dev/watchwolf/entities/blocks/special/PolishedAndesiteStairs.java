...
public class PolishedAndesiteStairs extends Block implements Orientable, Sectionable {
	...
	public Sectionable setSection(Sectionable.Section section) throws IllegalArgumentException {
		if (!this.allowedSections.contains(section)) throw new IllegalArgumentException("PolishedAndesiteStairs block doesn't allow the section " + section.name());
		PolishedAndesiteStairs current = new PolishedAndesiteStairs(this);
	...
	/*   --- CONSTRUCTORS ---   */
	public PolishedAndesiteStairs(short id) {
		super(id, "POLISHED_ANDESITE_STAIRS", (ins, f) -> {
