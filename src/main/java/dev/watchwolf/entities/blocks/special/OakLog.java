...
public class OakLog extends Block implements Directionable {
	...
	public Directionable setDirection(Directionable.Direction d) throws IllegalArgumentException {
		if (!this.allowedDirections.contains(d)) throw new IllegalArgumentException("OakLog block doesn't allow the direction " + d.name());
		OakLog current = new OakLog(this);
		current.direction = d;
		return current;
	}
	@Override
